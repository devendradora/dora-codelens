...
exports.TabbedWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const webview_utils_1 = require("./core/webview-utils");
const dedicated_analysis_view_manager_1 = require("./core/dedicated-analysis-view-manager");
class TabbedWebviewProvider {
    constructor(context, outputChannel) {
        ...
        this.dedicatedViewManager = new dedicated_analysis_view_manager_1.DedicatedAnalysisViewManager(context, outputChannel);
    ...
    showCurrentFileAnalysis(analysisData, view = "graph") {
        this.analysisData = this.createSafeDataStructure((0, webview_utils_1.convertCurrentFileAnalysisData)(analysisData));
        this.currentTab = view === "json" ? "codegraphjson" : "codegraph";
        this.log(`Showing current file analysis in ${this.currentTab} view`);
        this.createOrShowWebview();
        this.postUpdateData();
    }
    showTechStackGraph(analysisData) {
        this.log("Showing tech stack graph in tabbed view");
        this.analysisData = this.createSafeDataStructure((0, webview_utils_1.convertAnalysisDataForWebview)(analysisData));
        this.currentTab = "techstack";
        this.createOrShowWebview();
        this.postUpdateData();
    }
    showModuleGraph(analysisData) {
    ...
    showGitAnalytics(analysisData, analysisType) {
        this.log(`Showing Git analytics for ${analysisType}`);
        const webviewData = (0, webview_utils_1.convertGitAnalyticsData)(analysisData, analysisType);
    ...
    showTabLoadingState(tabId, message) {
        this.currentTab = tabId;
    ...
    showTabErrorState(tabId, error, showRetry = false) {
        this.currentTab = tabId;
        this.enqueueMessage({
            command: "updateError",
            tabId,
    ...
    showCallHierarchy(analysisData, functionName) {
     ...
     */
    async showDedicatedAnalysisView(viewType, analysisData) {
        ...
        try {
            ...
            this.logError(`Failed to show dedicated analysis view ${viewType}`, error);
    ...
    createOrShowWebview() {
        ...
        if (this.panel) {
            try {
                ...
                this.logError("Failed to reveal panel", e);
            }
            return;
        }
        this.log("Creating new webview panel");
        this.panel = vscode.window.createWebviewPanel("doracodebirdTabbedView", "DoraCodeBirdView", column, { enableScripts: true, retainContextWhenHidden: true });
        ...
        try {
            ...
            this.logError("Failed to reveal new panel", e);
        }
    }
    handleMessage(msg) {
        if (!msg || !msg.command) {
    ...
    async postUpdateData() {
        ...
        try {
            ...
            this.logError("Failed to prepare data update", error);
            this.enqueueMessage({
                command: "updateData",
                data: {
                    analysisData: {
    ...
    getHtml() {
        const nonce = (0, webview_utils_1.generateNonce)();
        const webview = this.panel?.webview;
        if (!webview) {
            return this.generateFallbackHtml("Webview not available");
        }
        try {
            const resourceUris = (0, webview_utils_1.getResourceUris)(this.context, webview);
            ...
            this.logError("Failed to generate HTML", error);
     ...
     */
    generateSimpleHtml(nonce, csp, resourceUris) {
        return `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8"/>
...
<body>
     ...
     */
    generateFallbackHtml(errorMessage) {
        return `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8"/>
...
exports.TabbedWebviewProvider = TabbedWebviewProvider;
//# sourceMappingURL=tabbed-webview-provider.js.map