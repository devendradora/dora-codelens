...
export class TestUtils {
     ...
     */
    static createTempWorkspace(): string {
        const tempDir = path.join(__dirname, `${TEST_CONFIG.TEMP_WORKSPACE_PREFIX}${Date.now()}`);
     ...
     */
    static cleanupTempWorkspace(workspaceDir: string): void {
        if (fs.existsSync(workspaceDir) && workspaceDir.includes(TEST_CONFIG.TEMP_WORKSPACE_PREFIX)) {
     ...
     */
    static createPythonFile(filePath: string, content: string): void {
     ...
     */
    static createMockPythonProject(workspaceDir: string, fileCount: number = TEST_CONFIG.SMALL_PROJECT_FILES): void {
        // Create main module
        const mainContent = `
...
`;
     ...
     */
    static createMockDjangoProject(workspaceDir: string): void {
...
`;
     ...
     */
    static async ensureExtensionActive(): Promise<vscode.Extension<any>> {
     ...
     */
    static async safeExecuteCommand(command: string, ...args: any[]): Promise<{ success: boolean; error?: Error }> {
        try {
...
export class TestAssertions {
     ...
     */
    static assertPerformance(executionTime: number, maxTime: number, operation: string): void {
     ...
     */
    static assertObjectStructure(obj: any, requiredProps: string[], message?: string): void {
