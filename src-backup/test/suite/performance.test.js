...
`;
        ...
        const pythonFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(tempWorkspaceDir, '**/*.py'), '**/node_modules/**', 100);
...
`;
        ...
        const discoveryStartTime = Date.now();
        const pythonFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(tempWorkspaceDir, '**/*.py'), '**/node_modules/**', 50);
        const discoveryTime = Date.now() - discoveryStartTime;
...
`;
        ...
        try {
            const document = await vscode.workspace.openTextDocument(vscode.Uri.file(path.join(tempWorkspaceDir, 'large_file.py')));
    ...
    test('Stress: Rapid command execution', async () => {
        ...
        for (let i = 0; i < commandCount; i++) {
            ...
            promises.push(Promise.resolve(vscode.commands.executeCommand(command)).catch(() => {
...
`;
    ...
    test('Performance: File system operations', async () => {
        ...
        // Test file discovery
        const discoveryStartTime = Date.now();
        const files = await vscode.workspace.findFiles(new vscode.RelativePattern(tempWorkspaceDir, '**/*.py'), '**/node_modules/**', fileCount + 10);
        const discoveryTime = Date.now() - discoveryStartTime;
