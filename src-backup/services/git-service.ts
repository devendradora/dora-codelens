...
import { AnalyzerRunner, AnalysisResult } from '../analyzer-runner';
...
export class GitService {
    ...
    constructor(
        ...
        this.analyzerRunner = new AnalyzerRunner(this.outputChannel, this.context.extensionPath);
    }

    /**
     * Run git author statistics analysis
     */
    public async runGitAuthorStatistics(): Promise<GitAnalysisResult> {
     ...
     */
    public async runGitModuleContributions(): Promise<GitAnalysisResult> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) {
            throw new Error('No workspace folder open');
        }
     ...
     */
    public async runGitCommitTimeline(): Promise<GitAnalysisResult> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) {
            throw new Error('No workspace folder open');
        }
     ...
     */
    private handleGitAnalysisError(result: GitAnalysisResult, analysisType: string): void {
        ...
        const hasGitError = errors.some(error => 
            error.message.toLowerCase().includes('not a git repository') ||
            error.message.toLowerCase().includes('git not found')
        );

        const hasPythonError = errors.some(error =>
            error.message.toLowerCase().includes('python not found') ||
        ...
        if (hasGitError) {
            ...
            vscode.window.showErrorMessage(
                'Python not found. Git analytics require Python to be installed.',
                'Open Settings',
                'Install Python',
                'View Output'
            ).then(action => {
            ...
            vscode.window.showErrorMessage(
                'Required Python dependencies are missing for Git analytics.',
            ...
            vscode.window.showErrorMessage(
                `${analysisType} analysis failed: ${errorMessage}`,
                'View Output',
                'Retry'
            ).then(action => {
     ...
     */
    private showGitDependencyInstallationGuide(): void {
