...
export class AnalyzerRunner {
  ...
  public async runAnalysis(
    ...
    try {
      ...
      this.log(
        `Starting Python analyzer: ${pythonPath} ${analyzerPath} ${args.join(
          " "
        )}`
      );
  ...
  public async runCurrentFileAnalysis(
    ...
    try {
      ...
      this.log(
        `Starting current file analysis: ${pythonPath} ${analyzerPath} ${args.join(
          " "
        )}`
      );
      ...
      // Execute the analyzer
      const result = await this.executeCurrentFileAnalyzer(
        pythonPath,
        analyzerPath,
        args,
        progress
  ...
  public async runGitAnalysis(
    ...
    try {
      ...
      this.log(
        `Starting Git analysis: ${pythonPath} ${analyzerPath} ${args.join(" ")}`
      );
      ...
      const result = await this.executeGitAnalyzer(
        pythonPath,
        analyzerPath,
        args,
        options,
  ...
  private async validateOptions(options: AnalyzerOptions): Promise<void> {
    ...
    if (!stats.isDirectory()) {
      throw new Error(
        `Project path is not a directory: ${options.projectPath}`
      );
    }
        ...
        if (
          ...
          if (executionDetails && executionDetails.execCommand) {
            ...
            if (await this.validatePythonPath(pythonPath)) {
              this.log(
                `Using Python path from Python extension: ${pythonPath}`
              );
              return pythonPath;
            }
          }
    ...
    return new Promise((resolve) => {
      ...
      process.on("close", (code) => {
        if (code === 0) {
          ...
          this.log(
            `Python validation failed for ${pythonPath}: exit code ${code}, stderr: ${stderr}`
          );
          resolve(false);
        }
      });
  ...
  private getCurrentFileAnalyzerScriptPath(): string {
    ...
    if (!fs.existsSync(analyzerPath)) {
      throw new Error(
        `Current file analyzer script not found at: ${analyzerPath}`
      );
    }
   ...
   */
  private buildGitAnalyzerArguments(options: {
    projectPath: string;
    analysisType: string;
  }): string[] {
  ...
  private parseAndValidateAnalyzerOutput(stdout: string): AnalysisResult {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON output from analyzer: ${
    ...
    if (!validationResult.isValid) {
      this.log(
        `Analyzer output validation warnings: ${validationResult.warnings.join(
          ", "
        )}`
      );
    }
   ...
   */
  private validateAnalyzerOutput(data: any): {
    isValid: boolean;
    warnings: string[];
  } {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        if (this.currentProcess) {
          this.currentProcess.kill("SIGTERM");
          reject(
            new Error(`Current file analysis timed out after ${timeout}ms`)
          );
        }
      }, timeout);
      ...
      this.currentProcess.on("close", (code) => {
        ...
        if (code === 0 || (code === 1 && stdout.trim())) {
          try {
            ...
            if (code === 1) {
              this.log(
                `Current file analyzer completed with warnings/errors but produced valid output`
              );
            }
            ...
            this.logError(
              "Failed to parse current file analyzer output",
              parseError
            );
            reject(
              new Error(
  ...
  private parseCurrentFileAnalyzerOutput(stdout: string): AnalysisResult {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON output from current file analyzer: ${
          error instanceof Error ? error.message : error
        }`
      );
    }
      ...
      this.currentProcess.on("close", (code) => {
        ...
        if (code === 0 || (code === 1 && stdout.trim())) {
          ...
          try {
            ...
            if (code === 1) {
              this.log(
                `Git analyzer completed with warnings/errors but produced valid output`
              );
            }
            ...
            reject(
              new Error(
                `Failed to parse Git analyzer output: ${
                  parseError instanceof Error ? parseError.message : parseError
                }`
              )
            );
  ...
  private parseGitAnalyzerOutput(stdout: string): AnalysisResult {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON output from Git analyzer: ${
          error instanceof Error ? error.message : error
        }`
      );
    }
  ...
  public async runDatabaseSchemaAnalysis(
    ...
    try {
      ...
      this.log(
        `Starting database schema analysis: ${pythonPath} ${analyzerPath} ${args.join(
          " "
        )}`
      );
      ...
      const result = await this.executeDatabaseSchemaAnalyzer(
        pythonPath,
        analyzerPath,
        args,
        progress
        ...
        if (code === 0) {
          try {
            ...
            if (result && typeof result === "object") {
              ...
              throw new Error(
                "Invalid result format from database schema analyzer"
              );
            }
          } catch (parseError) {
            this.logError(
              "Failed to parse database schema analyzer output",
              parseError
            );
            reject(
              new Error(
  ...
  private logError(message: string, error: any): void {
    const timestamp = new Date().toISOString();
    this.outputChannel.appendLine(
      `[${timestamp}] AnalyzerRunner ERROR: ${message}`
    );
    if (error) {
