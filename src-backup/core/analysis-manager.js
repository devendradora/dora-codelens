...
class AnalysisManager {
    constructor(analyzerRunner, configurationManager, outputChannel, statusBarItem) {
    ...
    async analyzeProject() {
        ...
        if (!configValidation.isValid) {
            ...
            vscode.window
                .showWarningMessage("Configuration issues detected:", { modal: true, detail: issues }, "Fix Settings")
                .then((action) => {
        ...
        try {
            ...
            const techStackResult = await this.analyzerRunner.analyzeTechStack({
                ...
                enableCaching: this.configurationManager.getConfiguration().enableCaching,
            });
            if (!techStackResult.success) {
                this.logError("Tech stack analysis failed", new Error(techStackResult.errors?.[0]?.message || "Unknown tech stack error"));
            ...
            const result = await vscode.window.withProgress({
                ...
                const options = this.configurationManager.getAnalyzerOptions(workspaceFolders[0].uri.fsPath);
            ...
            this.log(`Combined result created - success: ${combinedResult.success}, hasData: ${!!combinedResult.data}`);
            ...
            if (result.data) {
                this.log(`Analysis result data structure: ${JSON.stringify({
                    ...
                    moduleNodes: result.data && result.data.modules
                        ? result.data.modules.nodes?.length || 0
                        : 0,
                    hasFunctions: !!(result.data && result.data.functions),
                    functionNodes: result.data && result.data.functions
            ...
            if (result.success) {
                ...
                if (!this.hasValidAnalysisData(result)) {
                    const action = await vscode.window.showInformationMessage("Project analysis completed successfully!", "Show Code Graph", "View Output");
                    if (action === "Show Code Graph") {
                        // Don't call the command again - just let the UI manager handle showing the results
                        this.log("User requested to show code graph - results will be displayed by UI manager");
                    }
                ...
                if (hasValidData) {
                    ...
                    const action = await vscode.window.showWarningMessage("Project analysis completed with errors, but data is available.", "Show Code Graph", "View Output", "View Errors");
                    if (action === "Show Code Graph") {
                        // Don't call the command again - just let the UI manager handle showing the results
                        this.log("User requested to show code graph - results will be displayed by UI manager");
                    }
            ...
            this.log(`Returning analysis result - success: ${combinedResult.success}`);
    ...
    async analyzeDatabaseSchema() {
        try {
            ...
            return await vscode.window.withProgress({
                ...
                const result = await this.analyzerRunner.analyzeDatabaseSchema(options, progress, token);
                if (result.success) {
                    return { success: true, data: result.data || result };
                }
                else {
                    throw new Error(result.errors?.[0]?.message || "Failed to analyze database schema");
                }
            });
        }
        catch (error) {
    ...
    async showCallHierarchy(uri, position) {
        ...
        if (editor.document.languageId !== "python") {
            vscode.window.showErrorMessage("Call hierarchy is only available for Python files. Current file type: " +
        ...
        try {
            ...
            if (!functionInfo) {
                vscode.window
                    .showInformationMessage("No function found at cursor position. Please place cursor on a function name or definition.", "Help")
                    .then((action) => {
                    if (action === "Help") {
                        vscode.window.showInformationMessage('To show call hierarchy:\n1. Place cursor on a function name\n2. Right-click and select "Show Call Hierarchy"\n3. Or use the command palette: "DoraCodeBirdView: Show Call Hierarchy"');
                    }
                });
                return;
            }
            ...
            if (!this.hasValidFunctionData(this.state.lastResult)) {
                vscode.window
                    .showWarningMessage("No function data available for call hierarchy. The analysis may not have found any function calls.", "Re-run Analysis", "Check Output")
                    .then((action) => {
            ...
            this.log(`Call hierarchy request processed for function: ${functionInfo.fullName}`);
        }
        catch (error) {
            this.logError("Failed to show call hierarchy", error);
            vscode.window
                .showErrorMessage("Failed to show call hierarchy. Check output for details.", "Check Output", "Try Again")
                .then((action) => {
     ...
     */
    async runCurrentFileAnalysis(filePath, progress, token) {
        if (this.state.isAnalyzing) {
            vscode.window.showWarningMessage("Analysis is already in progress");
            return {
                success: false,
        ...
        try {
            ...
            const result = await this.analyzerRunner.runCurrentFileAnalysis(filePath, progress, token);
            if (result.success && result.data) {
                this.log(`Current file analysis completed successfully for: ${filePath}`);
    ...
    async runGitAnalysis(options, progress, token) {
        ...
        try {
            ...
            const result = await this.analyzerRunner.runGitAnalysis(gitOptions, progress, token);
            if (result.success && result.data) {
     ...
     */
    async runDatabaseSchemaAnalysis(options, progress, token) {
        if (this.state.isAnalyzing) {
            vscode.window.showWarningMessage("Analysis is already in progress");
            return {
                success: false,
        ...
        switch (serviceName) {
            case "currentFile":
                ...
                try {
                    ...
                    const extension = extensions.getExtension("doracodebird.doracodebird-view");
                    if (extension) {
                    ...
                    const analyzerPath = path.join(extensionPath, "analyzer", "current_file_analyzer.py");
            ...
            case "git":
                ...
                try {
                    ...
                    const analyzerPath = path.join(extensionPath, "analyzer", "git_analytics_runner.py");
                    return fs.existsSync(analyzerPath);
                }
                catch {
                    return false;
            ...
            case "database":
                ...
                try {
                    ...
                    const analyzerPath = path.join(extensionPath, "analyzer", "database_schema_analyzer.py");
                    return fs.existsSync(analyzerPath);
                }
                catch {
                    return false;
    ...
    handleAnalysisErrors(result) {
        ...
        if (commonErrors.pythonNotFound) {
            vscode.window
                .showErrorMessage("Python not found. Please install Python or configure the Python path in settings.", "Open Settings", "Install Python Guide")
                .then((action) => {
            ...
            vscode.window
                .showErrorMessage("Required Python dependencies are missing. Please install them.", "View Requirements", "Install Dependencies")
                .then((action) => {
            ...
            vscode.window
                .showWarningMessage(`Analysis completed with ${commonErrors.parsingErrors} parsing error(s). Some files may have syntax issues.`, "View Details")
                .then((action) => {
     ...
     */
    detectFunctionAtPosition(editor, position) {
     ...
     */
    showDependencyInstallationGuide() {
...
1. Navigate to the analyzer directory:
   cd ${vscode.extensions.getExtension("your-extension-id")?.extensionPath}/analyzer
...
3. Re-run the analysis`;
        vscode.window
            .showInformationMessage("Dependency Installation Guide", { modal: true, detail: message }, "Copy Commands")
            .then((action) => {
...
For more help, check the extension documentation or report an issue.`;
        vscode.window
            .showInformationMessage("Troubleshooting Guide", { modal: true, detail: message }, "Open Documentation", "Report Issue")
            .then((action) => {
