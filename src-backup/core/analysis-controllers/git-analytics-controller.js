...
exports.GitAnalyticsController = void 0;
...
class GitAnalyticsController {
     ...
     */
    createGitAnalyticsGraphData(data) {
        const enhancedData = {
            modules: {
     ...
     * Create contributor nodes
     */
    createContributorNodes(enhancedData, contributors) {
        for (const contributor of contributors) {
            ...
            const contributorNode = {
                ...
                module: 'contributors',
                complexity: {
                    cyclomaticComplexity: Math.min(commits / 10, 10),
     ...
     * Create file hotspot nodes
     */
    createFileHotspotNodes(enhancedData, fileChanges) {
     ...
     * Create commit timeline nodes
     */
    createCommitTimelineNodes(enhancedData, commits) {
        ...
        for (const [timeKey, timeCommits] of Object.entries(timeGroups)) {
            ...
            const commitCount = timeCommits.length;
            const timelineNode = {
                id: timeId,
     ...
     */
    createCollaborationEdges(enhancedData, data) {
        ...
        if (data.fileChanges && data.contributors) {
            for (const fileChange of data.fileChanges) {
                ...
                const fileContributors = data.contributors.filter((c) => fileChange.authors?.includes(c.name) || fileChange.authors?.includes(c.author));
     ...
     */
    enhanceForGitAnalytics(enhancedData, originalData) {
        // Add git-specific metadata
        enhancedData.metadata.gitAnalytics = {
     ...
     */
    calculateGitStatistics(enhancedData, data) {
     ...
     */
    getContributorComplexityLevel(commits) {
        if (commits <= 10)
            return 'low';
        if (commits <= 50)
            return 'medium';
     ...
     */
    getHotspotComplexityLevel(changes) {
        if (changes <= 5)
            return 'low';
        if (changes <= 15)
            return 'medium';
     ...
     */
    getTimelineComplexityLevel(commits) {
        if (commits <= 5)
            return 'low';
        if (commits <= 20)
            return 'medium';
     ...
     */
    async handleContributorClick(interaction, state) {
     ...
     */
    async handleFileHotspotClick(interaction, state) {
     ...
     */
    async handleTimeFilter(interaction, state) {
     ...
     */
    async handleCommitTimeline(interaction, state) {
     ...
     */
    async handleBranchAnalysis(interaction, state) {
...
exports.GitAnalyticsController = GitAnalyticsController;
//# sourceMappingURL=git-analytics-controller.js.map