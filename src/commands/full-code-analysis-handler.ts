...
export class FullCodeAnalysisHandler {
  ...
  /**
   * Execute full code analysis with duplicate call prevention and caching
   */
    ...
    return this.duplicateCallGuard.executeWithProtection(
      ...
      async () => {
        ...
        if (!options.forceRefresh) {
          const cachedResult = this.stateManager.getCachedResult(workspacePath);
          if (cachedResult) {
            this.errorHandler.logError(
            ...
            // Show cached results in webview
            try {
              this.webviewManager.showFullCodeAnalysis(cachedResult.data);
            } catch (webviewError) {
              this.errorHandler.logError(
                "Failed to show cached webview",
                webviewError,
                FullCodeAnalysisHandler.COMMAND_ID
              );
            }
            ...
            vscode.window.showInformationMessage(
              `Showing cached analysis results (${ageMinutes} minutes old). Use refresh button for latest analysis.`
            );
        ...
        try {
          ...
          return vscode.window.withProgress(
            {
              ...
              title: options.forceRefresh
                ? "Refreshing Full Code Analysis"
            ...
            async (progress, token) => {
              ...
              const result = await this.executePythonAnalysis(
                workspacePath,
                options,
                progress
              );
              ...
              const validatedResult =
                this.errorHandler.validateAnalysisResult(result);
              if (!validatedResult) {
                throw new Error("Analysis returned invalid result");
              }

              // Cache the result
              this.stateManager.setCachedResult(
                validatedResult,
                workspacePath,
                options
              );
              ...
              this.errorHandler.logError(
                "Full code analysis completed successfully",
                null,
                FullCodeAnalysisHandler.COMMAND_ID
              );
              ...
              try {
                ...
                this.errorHandler.logError(
                  "Failed to show webview, analysis completed but display failed",
                  webviewError,
                  FullCodeAnalysisHandler.COMMAND_ID
                );
                vscode.window.showWarningMessage(
                  "Analysis completed but failed to display results. Check the output for details."
                );
              }
              ...
              const message = options.forceRefresh
                ? `Analysis refreshed! Found ${
                    validatedResult.code_graph_json?.length || 0
                  } modules.`
                : `Full code analysis completed! Found ${
                    validatedResult.code_graph_json?.length || 0
                  } modules.`;
          ...
          this.stateManager.removeActiveCommand(
            FullCodeAnalysisHandler.COMMAND_ID
          );
        ...
        reject(
          new Error(
            `Analysis timed out after ${
              FullCodeAnalysisHandler.PYTHON_TIMEOUT / 1000
            } seconds`
          )
        );
  ...
  private cancelCurrentProcess(): void {
    if (this.currentProcess) {
      this.errorHandler.logError(
        "Cancelling Python analysis process",
        null,
        FullCodeAnalysisHandler.COMMAND_ID
      );
  ...
  private getAnalyzerPath(): string {
    ...
    return path.join(
      extensionPath,
      "analyzer",
      FullCodeAnalysisHandler.PYTHON_SCRIPT
    );
  }

  /**
   * Execute refresh analysis (force refresh)
   */
  public async executeRefresh(): Promise<any> {
