...
export class BackgroundAnalysisManager {
  ...
  public static getInstance(errorHandler?: ErrorHandler): BackgroundAnalysisManager {
    if (!BackgroundAnalysisManager.instance) {
      if (!errorHandler) {
        throw new Error('ErrorHandler required for first initialization');
      }
      BackgroundAnalysisManager.instance = new BackgroundAnalysisManager(errorHandler);
    }
   ...
   */
  public async analyzeFileInBackground(document: vscode.TextDocument): Promise<AnalysisResult> {
   ...
   */
  public getCachedAnalysis(filePath: string, content?: string): AnalysisResult | null {
   ...
   * Set cached analysis
   */
  public setCachedAnalysis(filePath: string, content: string, result: AnalysisResult): void {
      ...
      async (progress) => {
        ...
        try {
          ...
          if (!this.currentFileAnalysisHandler) {
            ...
            const analysisData = await this.executeDirectPythonAnalysis(filePath);
            ...
            const analysisData = await this.currentFileAnalysisHandler.executeInBackground(filePath);
    ...
    if (analysisData.functions && Array.isArray(analysisData.functions)) {
      for (const func of analysisData.functions) {
        functions.push({
          ...
          cyclomatic_complexity: func.cyclomatic_complexity || func.complexity || 0,
        ...
        if (cls.methods && Array.isArray(cls.methods)) {
          for (const method of cls.methods) {
            methods.push({
              ...
              complexity: method.complexity || method.cyclomatic_complexity || 0,
              cyclomatic_complexity: method.cyclomatic_complexity || method.complexity || 0,
    ...
    const onDidChangeTextDocument = vscode.workspace.onDidChangeTextDocument((event) => {
      if (event.document.languageId === 'python' && event.contentChanges.length > 0) {
        ...
        const hasSignificantChanges = event.contentChanges.some(change => 
          change.text.trim().length > 0 || change.rangeLength > 10
        );
   ...
   */
  public setCurrentFileAnalysisHandler(handler: CurrentFileAnalysisHandler): void {
  ...
  private async executeDirectPythonAnalysis(filePath: string): Promise<any> {
    ...
    const analyzerPath = pythonService.getAnalyzerScriptPath('current_file_analyzer.py');
    
    // Execute Python script
    const result = await pythonService.executePythonScriptForJSON(analyzerPath, {
      args: [filePath],
      timeout: 30000
    });
