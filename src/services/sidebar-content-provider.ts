  ...
  implements vscode.TreeDataProvider<SidebarTreeItem>
{
  ...
  /**
   * Get the path to the custom Dora Code Lens icon
   */
  private getDoraIcon(): string {
    return path.join(
      this.extensionPath,
      "resources",
      "dora-code-lens-kiro.png"
    );
  }
  ...
  private getRootSections(): SidebarTreeItem[] {
    ...
    // Quick Actions section - using custom Dora icon
    sections.push(
      new SidebarTreeItem(
        "Quick Actions",
    ...
    // Recent Analysis section - expanded by default to show increased log size, using custom Dora icon
    sections.push(
      new SidebarTreeItem(
        "Recent Analysis",
        vscode.TreeItemCollapsibleState.Expanded,
        "recentAnalysis",
        undefined,
        "History of recent analysis operations (showing last 15 entries)",
        this.getDoraIcon()
      )
    );
  ...
  private getQuickActionItems(): SidebarTreeItem[] {
    ...
    if (isPythonFile) {
      // Current file analysis - using custom Dora icon
      actions.push(
        new SidebarTreeItem(
    ...
    // Full project analysis - using custom Dora icon
    actions.push(
      new SidebarTreeItem(
  ...
  private getRecentAnalysisItems(): SidebarTreeItem[] {
    ...
    // Show last 15 analyses (increased from 5)
    const recentItems = this.recentAnalysis.slice(0, 15);
    ...
    for (const analysis of recentItems) {
      ...
      if (analysis.filePath) {
        ...
        const dirName = path.basename(path.dirname(analysis.filePath));
  ...
  public addRecentAnalysis(entry: RecentAnalysisEntry): void {
    ...
    // Keep only last 25 entries (increased from 10)
    if (this.recentAnalysis.length > 25) {
      this.recentAnalysis = this.recentAnalysis.slice(0, 25);
    }
